...
void UART1_InList(char *bufPt, unsigned short max){
int length=0;
char character, prevchar, prev2char, prev3char, prev4char;
  character = UART1_InChar();
  while(!((character == LF) && (prevchar == CR) && (prev2char == 'D') && (prev3char == 'N') && (prev4char == 'E'))){
    if(character == BS){
      if(length){
        bufPt--;
        length--;
